...
- (NSArray *)mas_distributeVerticalViewsWithFixedItemWidth:(CGFloat)fixedItemWidth
                                           fixedItemHeight:(CGFloat)fixedItemHeight
                                          fixedLineSpacing:(CGFloat)fixedLineSpacing {
    if (self.count < 1) {
        return self.copy;
    ...
    NSInteger rowCount = tempViews.count;
    
    MAS_VIEW *prev;
    for (int i = 0; i < tempViews.count; i++) {
        
        MAS_VIEW *v = tempViews[i];
        NSInteger currentRow = i;
        ...
        [v mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            if (currentRow == rowCount - 1) {
                if (currentRow != 0) {
                    make.top.equalTo(prev.mas_bottom).offset(fixedLineSpacing);
                }
